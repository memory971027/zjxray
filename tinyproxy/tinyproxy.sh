...
InstallFiles() {
	...
	$download_tool_cmd tinyproxy http://yourdomain.com/resources/tinyproxy/${tinyproxy_UPX}/linux_${machine} || Error "tinyproxy download failed."
	$download_tool_cmd tinyproxy.init http://yourdomain.com/resources/tinyproxy/tinyproxy.init || Error "tinyproxy.init download failed."
	...
	if type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/tinyproxy.service http://yourdomain.com/resources/tinyproxy/tinyproxy.service || Error "tinyproxy.service download failed."
