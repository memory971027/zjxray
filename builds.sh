...
tinyproxy_task() {
	if $download_tool_cmd tinyproxy.sh ./builds/tinyproxy.sh; then
...
cns_task() {
	if $download_tool_cmd cns.sh ./builds/cns.sh; then
		chmod 777 cns.sh
...
xray_task() {
	if $download_tool_cmd xray.sh ./builds/xray.sh; then
		chmod 777 xray.sh
...
amy4Server_task() {
	if $download_tool_cmd amy4Server.sh ./builds/amy4Server.sh; then
