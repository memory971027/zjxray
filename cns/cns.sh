...
InstallFiles() {
	...
	$download_tool_cmd cns ../binaries/cns/${cns_UPX}/linux_${machine}${softfloat} || Error "cns download failed."
	$download_tool_cmd cns.init ../init-scripts/cns.init || Error "cns.init download failed."
	...
	if type systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/cns.service ../init-scripts/cns.service || Error "cns.service download failed."
